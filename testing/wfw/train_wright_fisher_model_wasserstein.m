function train_wright_fisher_model_wasserstein
...
sig = 1;
theta_f_mu = zeros(size(theta_f'));
theta_g_mu = zeros(size(theta_g));
...
%     cEpoch
    ...
    for i = 1:nSamp        
        [Zs1, Xs1, Pis1, log_Ps1] = wright_fisher_fwd(N,T,nSim,theta_f_samps(i,:)',theta_h,theta_z0,theta_g_samps(:,:,i),bin_expr_flag,fwd_sigma,sd,verbose);
        for j = 1:nSim
            sims_Zs{i,j} = Zs1{j};
            sims_Xs{i,j} = Xs1{j};
        end
...
save('training_outputs','theta_f_mus_all','theta_g_mus_all','sigs_all','scs_all',...
    'Fs_all','theta_f_samps_all','theta_g_samps_all',...
    'sims_Zs_all','sims_Xs_all','Zs', 'Xs', 'Pis', 'log_Ps', 'emds_all',...
    'theta_f_mu','theta_g_mu','sig');
